...
class RefineCoach:
    def __init__(self, opts, prev_train_checkpoint=None):
        ...
        self.train_dataloader = DataLoader(self.train_dataset,
                                           ...
                                           drop_last=True)
        self.target_dataloader = DataLoader(self.test_dataset,
                                          batch_size=1,
                                          shuffle=False)
        ...
        self.prev_train_checkpoint = prev_train_checkpoint


    def insert_target(self, target_image, batch):
        batch[0] = torch.cat((batch[0], target_image[0]), 0)
        batch[1] = torch.cat((batch[1], target_image[1]), 0)
        return batch
    ...
    def restart_training(self):
        self.load_from_train_checkpoint(self.prev_train_checkpoint)


    def train(self):
        self.net.train()
        for target_image in self.target_dataloader:
            ...
            file_name = target_image[2][0]
            for batch_idx, batch in enumerate(self.train_dataloader):


                loss_dict = {}

                batch = self.insert_target(target_image, batch)
                ...
                local_step += 1
                if local_step > self.opts.max_steps:
                    y_hat_1024 = self.net.forward(torch.unsqueeze(x[-1], 0), resize=False)
                    torch.save(torch.unsqueeze(latent[-1], 0).cpu(), f'saved_w/{file_name}_{local_step-1}.pt')
                    common.save_image(y_hat_1024, f'saved_w/{file_name}_{local_step-1}.jpg')
                    break
    ...
    def parse_and_log_images(self, id_logs, x, y, y_hat, title, subscript=None,
                             display_count=1):
        im_data = []
