...
from training.refine_coach import RefineCoach
import pdb


def main():
    opts = RefineOptions().parse()
    previous_train_ckpt = None
    opts, previous_train_ckpt = load_train_checkpoint(opts)

    coach = RefineCoach(opts, previous_train_ckpt)
    coach.train()
...
def load_train_checkpoint(opts):
    ...
    opts = update_new_configs(opts, new_opts_dict)
    pprint.pprint(opts)
    opts = Namespace(**opts)
