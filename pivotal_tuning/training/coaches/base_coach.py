...
class BaseCoach:
    def __init__(self, data_loader, use_wandb):
        ...
        if hyperparameters.first_inv_type == 'cycle':
            self.initilize_e4e()
    ...
    def get_inversion(self, w_path_dir, image_name, image):
        ...
        if not hyperparameters.use_saved_w_pivots or w_pivot is None:
    ...
    def load_inversions_new(self, path_dir, image_name, step):

        w_potential_path = f'{path_dir}/{image_name}_{step}.pt'
        while not os.path.isfile(w_potential_path):
            print(f'{image_name}_{step}.pt is not ready')
            time.sleep(30)
        time.sleep(3)
        w = torch.load(w_potential_path).cuda()
        self.w_pivots[image_name+"_"+str(step)] = w
        return w
    ...
    def calc_loss(self, generated_images, real_images, log_name, new_G,
                  use_ball_holder, w_batch):
        loss = 0.0

        if hyperparameters.pt_l2_lambda > 0:
            l2_loss_val = l2_loss.l2_loss(generated_images, real_images)
            if self.use_wandb:
                wandb.log({f'MSE_loss_val_{log_name}': l2_loss_val.detach().cpu()},
        ...
        if hyperparameters.pt_lpips_lambda > 0:
            ...
            if self.use_wandb:
                wandb.log({f'LPIPS_loss_val_{log_name}': loss_lpips.detach().cpu()},
                          step=global_config.training_step)
            loss += loss_lpips * hyperparameters.pt_lpips_lambda

        if use_ball_holder and hyperparameters.use_locality_regularization:
            ball_holder_loss_val = self.space_regulizer.space_regulizer_loss(new_G,
                                    w_batch, use_wandb=self.use_wandb)
