...
class Coach:
    def __init__(self, opts, prev_train_checkpoint=None):
        ...
        self.setup_inverse_progressive_steps(opts.inverse_progressive_start,
                                             opts.inverse_progressive_step_every)
    ...
    def setup_inverse_progressive_steps(self, start, progressive_step_every):
        self.opts.inverse_progressive_steps = []
        for i in range(18):
            self.opts.inverse_progressive_steps.append(start + (i - 1) * progressive_step_every)
    ...
    def load_from_train_checkpoint(self, ckpt):
        ...
        if self.opts.progressive_steps:
            ...
            self.check_for_inverse_progressive_training_update(is_resume_from_ckpt=True)
        print(f'Resuming training from step {self.global_step}')

    def train(self):
        self.net.train()
        if self.opts.progressive_steps:
            self.check_for_inverse_progressive_training_update()
        print(f"STAGE:{self.net.encoder.progressive_stage}")
    ...
    def check_for_inverse_progressive_training_update(self, is_resume_from_ckpt=False):
        for i in range(len(self.opts.inverse_progressive_steps)):
            if is_resume_from_ckpt and self.global_step >= self.opts.inverse_progressive_steps[i]:  # Case checkpoint
                self.net.encoder.set_progressive_stage(ProgressiveStage(17 - i))
            if self.global_step == self.opts.inverse_progressive_steps[i]:   # Case training reached progressive step
                self.net.encoder.set_progressive_stage(ProgressiveStage(17 - i))
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Discriminator ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ #
    ...
    def progressive_delta_lambda(self):
        if self.global_step % self.opts.delta_norm_lambda_progressive_step_every == 5:
            if self.global_step < self.opts.inverse_progressive_start:
                self.opts.delta_norm_lambda *= 1.2
            self.opts.w_discriminator_lambda /= 2

            print(f'delta_norm_lambda is updated to {self.opts.delta_norm_lambda}.')
            print(f'w_discriminator_lambda is updated to {self.opts.w_discriminator_lambda}.')
